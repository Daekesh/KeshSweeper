...
const TArray< FText > SKeshSweeperTab::StatusText( std::initializer_list< FText >( {
...
FText SKeshSweeperTab::GetCurrentGameStatusText() const
{
	...
	return SKeshSweeperTab::StatusText[ View->GetPlugin()->GetController()->GetGameStatus() ];
}

void SKeshSweeperTab::FixWidgets()
{
	if ( !View.IsValid() )
		return;
	...
	if ( GameWindow.IsValid() && View.IsValid() && View->GetPlugin().IsValid() && View->GetPlugin()->GetModel().IsValid() )
	{
